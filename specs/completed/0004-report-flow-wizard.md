...
- 2026-06-10: A-04 완료 화면의 "이후 처리"(R2 업로드·SHA-256 봉인 / 검토 큐 등록 / 검증 후 공개 반영) 안내 박스 제거. (요청: 채팅)
- 2026-06-10: A-03 첨부 드롭존에 드래그&드롭 업로드 추가 — 드롭 시 `handleFiles`로 처리(클릭 선택과 동일 경로), 드래그 중 시각 피드백 + "드래그&드롭" 안내. (요청: 채팅)
